...
def activity_level(trans_df: pd.DataFrame, lag: int) -> pd.DataFrame:
    ...
    trans_df[f"loc_delta_t_minus_{lag}"] = trans_df.groupby("cc_num") \
        .apply(lambda x: haversine_distance(x["longitude"], x["latitude"], x["longitude"].shift(-lag),
...
def aggregate_activity_by_hour(trans_df: pd.DataFrame, window_len) -> pd.DataFrame:
    ...
    cc_group = trans_df[["cc_num", "loc_delta_t_minus_1", "datetime"]].groupby("cc_num").rolling(window_len,
                                                                                                 on="datetime").mean()
    ...
    window_aggs_df = window_aggs_df.merge(trans_df[["cc_num", "datetime", "month"]].sort_index(), left_index=True,
                                          right_index=True)
